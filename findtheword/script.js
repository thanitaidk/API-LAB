...
$(document).ready(function () {
    ...
    function Pressure() { //activating timer, losing point when reaching 0
        var Press = setInterval(function () { //the different colors for each second
            Counter--;
            console.log(Counter);
            if (Counter == 5) {
            ...
            if (Counter == 0) { //recounts, 0 = 5
                ...
                document.getElementById("Points").innerHTML = "Score: " + CurrentScore; //announcing score
    ...
    interact('.dropzone').dropzone({ //dropzone only visible when interacting with a word
        accept: '.yes-drop',
        overlap: 0.5, //atleast 50% overlap to work 
        ...
        ondragleave: function (event) { //leaving dropzone
        ...
        ondrop: function (event) {
            if (event.relatedTarget.textContent == document.getElementById("Words").innerHTML) { //if dragged word matches chosen word
                ...
                if (Array.length == 0) { //winning the game when all words have been cleared out 
    ...
    //all the words
    var Array = ['net', 'float', 'java', 'abstract', 'else', 'switch', 'byte', 'hello',
        'break', 'if', 'delete', 'continue', 'null', 'frame', 'alert', 'close', 'set',
        'package', 'class', 'rate', 'page', 'default', 'parent', 'reset', 'case', 'x',
        'public', 'catch', 'function', 'private', 'void', 'bug', 'return', 'document',
        'while', 'double', 'type', 'import', 'all', 'button', 'date', 'screen', 'y',
        'object', 'loop', 'escape', 'form', 'open', 'value', 'click', 'over', 'target',
        'code', 'element', 'link', 'layer', 'image', 'assign', 'infinite', 'world',
        'window', 'term', 'variable', 'const', 'true', 'protected', 'new', 'style',
        'submit', 'key', 'length', 'extend', 'character', 'let', 'instance', 'final',
        'area', 'get', 'false', 'zero', 'pass', 'refresh', 'for', 'hidden', 'end'
    ];

    var str = Array.join(' ');
    var words = str.split(" "); //splitting the words with space between them (used before changing to ',')
    var text, i;
    text = "";
